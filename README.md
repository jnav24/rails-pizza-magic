...
```sh
git clone https://github.com/nickjj/docker-rails-example rails_pizza_magic
cd rails_pizza_magic
...
The app is named `rails_pizza_magic` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "rails_pizza_magic" and update a few Docker related resources.
